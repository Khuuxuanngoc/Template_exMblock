...
#ifdef Blynk_I2C__H_4_y241109
// #define Blynk_I2C__H_4_y241109
...
void BlynkGate::checkUartThenSendI2C4()
{ // Run send 100 bytes to I2C
...
void BlynkGate::SetupCharArrayToBuffer4(char *inputCharArray, char *tempArrChar, uint8_t len)
{ // Run send 100 bytes to I2C
    int timeSend = (len) / KXN_BYTE_PER_TIME_SEND;
...
void BlynkGate::checkI2CThenSendSerial()
{
    ...
    while (!isEmptyData)
    {
        ...
        Wire.requestFrom(slaveAddress, 32); // request 32 bytes from slave device #slaveAddress
    ...
    if (dataString.length() > 0)
    {
        ...
        int index = dataString.indexOf(BLYNK_I2C_CMD_VIRTUAL_PIN_RX);
        while (index != -1)
        {
            // Tạo chuỗi con của dataString
            String tempSubDataString = dataString.substring(index);

            // Xử lý chuỗi tại vị trí index
            String tempVirtualPin = splitString(tempSubDataString, BLYNK_I2C_CMD_VIRTUAL_PIN_RX, " ", " ", 0);
            String valueTemp = splitString(tempSubDataString, BLYNK_I2C_CMD_VIRTUAL_PIN_RX, " ", " ", 1);
            ...
            index = dataString.indexOf(BLYNK_I2C_CMD_VIRTUAL_PIN_RX, index + 1);
        }
    }
}
...
void BlynkGate::checkSendVirtualWrite()
{
    if (millis() - lastTimeVirtualWrite >= 2000)
    {
        ...
        {
            STRUCT_QUEUE_BLYNK_GATE(BlynkGateQueue)
            tempValue = DEQUEUE_BLYNK_GATE(BlynkGateQueue);
